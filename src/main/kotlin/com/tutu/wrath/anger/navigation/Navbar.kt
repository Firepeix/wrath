...
fun Container.navbar() {
    nav(className = "relative flex w-full flex-wrap items-center justify-between py-3 text-neutral-50 shadow-lg lg:flex-wrap lg:justify-start px-4") {
        background = Background(color = Color("#f44336"))
        ...
        div(className = "flex-1 px-4") { link("Ebisu", className = "btn btn-ghost normal-case text-xl font-bold") }
        div(className = "flex-none") { button("", className = "btn btn-square btn-ghost") {
            paddingBottom = CssSize(4, UNIT.px)
            customTag("svg", className = "inline-block w-5 h-5 stroke-current", attributes = mapOf(
                "xmlns" to "http://www.w3.org/2000/svg",
                "viewBox" to "0 0 24 24",
                "fill" to "none",
